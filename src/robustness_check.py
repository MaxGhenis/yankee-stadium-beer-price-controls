...
def run_sensitivity_analysis():
    ...
    for epsilon in elasticities:
        ...
        profit_change = ceil_metrics['profit'] - base_metrics['profit']
        
        # Calculate Welfare Change (using standard costs)
        welfare_base = model.social_welfare(base_ticket, base_beer)
...
def plot_robustness(df):
    ...
    plt.plot(df['cross_elasticity'], df['ticket_change'], 
             'o-', color='#003087', linewidth=2, label='Ticket Price Increase ($)')
    ...
    plt.title('Robustness Check: Ticket Prices Rise Even With Weak Complementarity', 
