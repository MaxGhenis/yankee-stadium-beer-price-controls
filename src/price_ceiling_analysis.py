...
def create_charts(df: pd.DataFrame, output_dir: Path = None):
    ...
    ax2.set_title('Beer Price Tracks Ceiling (When Binding)',
                  fontsize=13, fontweight='bold')
    ...
    ax2.axhline(baseline['total_beers']/1000, color='gray', linestyle='--', alpha=0.5, label='Baseline')
    ax2.axvline(12.5, color='gray', linestyle='--', alpha=0.5)
    ax2.set_xlabel('Beer Price Ceiling ($)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Total Beer Consumption (thousands)', fontsize=12, fontweight='bold')
    ax2.set_title('Lower Ceilings Increase Beer Consumption\n(Despite Lower Attendance)',
                  fontsize=13, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    ...
    ax2.axhline(baseline['consumer_surplus']/1e6, color='gray', linestyle='--', alpha=0.5, label='Baseline')
    ax2.axvline(12.5, color='gray', linestyle='--', alpha=0.5)
    ...
    ax2.set_title('Consumer Surplus Ambiguous\n(Cheaper Beer vs Higher Tickets)', fontsize=12, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ax2.legend()

    # Externality cost
    ax3.plot(df['beer_ceiling'], df['externality_cost']/1e6, 'o-', color='#d62728', linewidth=2)
    ax3.axhline(baseline['externality_cost']/1e6, color='gray', linestyle='--', alpha=0.5, label='Baseline')
    ...
    ax3.set_title('Externalities Rise with Lower Ceilings\n(More Consumption)', fontsize=12, fontweight='bold')
    ax3.grid(True, alpha=0.3)
    ax3.legend()

    # Social welfare
    ax4.plot(df['beer_ceiling'], df['social_welfare']/1e6, 'o-', color='#9467bd', linewidth=2)
    ax4.axhline(baseline['social_welfare']/1e6, color='gray', linestyle='--', alpha=0.5, label='Baseline')
    ...
    ax.plot(df['beer_ceiling'], df['consumer_surplus']/1e6, 'o-',
            color='#1f77b4', linewidth=2, label='Consumer Surplus', markersize=6)
    ax.plot(df['beer_ceiling'], df['producer_surplus']/1e6, 's-',
            color='#2ca02c', linewidth=2, label='Producer Surplus (Profit)', markersize=6)
    ax.plot(df['beer_ceiling'], -df['externality_cost']/1e6, '^-',
            color='#d62728', linewidth=2, label='Externality Cost (negative)', markersize=6)
    ax.plot(df['beer_ceiling'], df['social_welfare']/1e6, 'D-',
            color='#9467bd', linewidth=3, label='Social Welfare (sum)', markersize=7)
    ...
    ax.set_title('Welfare Decomposition: Lower Ceilings Hurt Stadium More Than They Help Consumers',
                 fontsize=13, fontweight='bold')
    ax.legend(loc='best', fontsize=11)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('Lower Ceilings Dramatically Increase Per-Fan Consumption',
                 fontsize=13, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def print_key_results(df: pd.DataFrame):
    ...
    for name, col, prefix in metrics:
        ...
        if prefix == '$' and abs(base_val) > 1000:
            ...
            print(f"{name:<30} ${base_val/1e6:>8.2f}M      ${ceil_val/1e6:>8.2f}M      {change/1e6:>+8.2f}M ({pct_change:>+5.1f}%)")
        elif prefix == '$':
            print(f"{name:<30} ${base_val:>8.2f}       ${ceil_val:>8.2f}       ${change:>+8.2f} ({pct_change:>+5.1f}%)")
        else:
            print(f"{name:<30} {base_val:>10,.0f}     {ceil_val:>10,.0f}     {change:>+10,.0f} ({pct_change:>+5.1f}%)")
