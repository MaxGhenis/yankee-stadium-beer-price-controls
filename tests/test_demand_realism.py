...
class TestRealisticConsumption:
    ...
    def test_very_cheap_beer_reasonable(self, model):
        ...
        assert (
            result["beers_per_fan"] < 10
        ), rf"Beers/fan = {result['beers_per_fan']:.1f} too high even at \$1"
    ...
    def test_five_dollar_beer_reasonable(self, model):
        ...
        assert (
            1.5 <= result["beers_per_fan"] <= 5.0
        ), rf"Beers/fan = {result['beers_per_fan']:.1f} unreasonable at \$5"
    ...
    def test_expensive_beer_low_consumption(self, model):
        ...
        assert (
            result["beers_per_fan"] < 1.0
        ), rf"Beers/fan = {result['beers_per_fan']:.1f} too high at \$20"
    ...
    def test_attendance_respects_capacity(self, model):
        ...
        for ticket_p in [10, 50, 100]:
            for beer_p in [0.50, 5, 15]:
                ...
                ), rf"Attendance {result['attendance']:,.0f} exceeds capacity {model.capacity:,.0f} at T=\${ticket_p}, B=\${beer_p}"
    ...
    def test_total_beers_respects_capacity(self, model):
        ...
        assert (
            result["total_beers"] <= max_possible
...
class TestDemandFunctionalForm:
    ...
    def test_consumption_smooth_not_kinked(self, model):
        ...
        assert cv < 2.0, f"Consumption changes too erratic (CV={cv:.2f}), demand may have kinks"
    ...
    def test_price_elasticity_reasonable_range(self, model):
        ...
        assert -3.0 <= elasticity <= -0.3, f"Elasticity {elasticity:.2f} outside plausible range"
