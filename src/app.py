...
beer_cost = st.sidebar.slider(
    "Marginal Cost per Beer ($)", min_value=0.5, max_value=10.0, value=2.0, step=0.5
)
...
if len(ceiling_7_scenarios) > 0 or price_ceiling <= 7.5:
    ...
    multiplier = (
        abs(ticket_change / (price_ceiling - base_beer_price))
        if price_ceiling != base_beer_price
        else 0
    )
...
with col1:
    st.metric(
        "Baseline Attendance", f"{baseline['attendance']:,.0f}", help="Attendance at current prices"
    )
...
with col3:
    st.metric(
        "Baseline Profit", f"${baseline['profit']:,.0f}", help="Stadium profit at current prices"
    )
...
with tab2:
    ...
    with col1:
        ...
        fig_attendance.add_trace(
            go.Bar(x=results_df["scenario"], y=results_df["attendance"], marker_color="#003087")
        )
    ...
    with col2:
        ...
        fig_beers = go.Figure()
        fig_beers.add_trace(
            go.Bar(x=results_df["scenario"], y=results_df["total_beers"], marker_color="#E4002B")
        )
    ...
    fig_per_fan.add_trace(
        go.Bar(x=results_df["scenario"], y=results_df["beers_per_fan"], marker_color="orange")
    )
...
with tab3:
    ...
    fig_prices.update_layout(
        ...
        yaxis2=dict(title="Beer Price ($)", overlaying="y", side="right"),
        height=500,
        hovermode="x unified",
    )
...
with tab4:
    ...
    fig_ext = go.Figure()
    fig_ext.add_trace(
        go.Bar(x=results_df["scenario"], y=results_df["externality_cost"], marker_color="darkred")
    )
...
with tab5:
    ...
    with col1:
        ...
        fig_ticket_response.add_vline(x=base_beer_price, line_dash="dash", line_color="gray")
    ...
    with col2:
        ...
        fig_beer_response.add_vline(x=base_beer_price, line_dash="dash", line_color="gray")
    ...
    fig_welfare_all.add_vline(
        x=base_beer_price, line_dash="dash", line_color="gray", annotation_text="Baseline"
    )
...
with col1:
    ...
    max_profit_row = results_df[
        results_df["scenario"] == summary["profit_maximizing_scenario"]
    ].iloc[0]
...
with col2:
    ...
    max_welfare_row = results_df[
        results_df["scenario"] == summary["welfare_maximizing_scenario"]
    ].iloc[0]
...
with col3:
    ...
    min_ext_row = results_df[results_df["scenario"] == summary["lowest_externality_scenario"]].iloc[
        0
    ]
