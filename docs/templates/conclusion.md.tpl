...
Stadium beer at \$12.50 is approximately profit-maximizing (\${{ baseline_beer }} optimal) when accounting for:
- Taxes (\${{ current_taxes_per_beer_raw }}/beer)
- Production costs (\${{ beer_cost_raw }}/beer)
- **Internalized costs** (varies by volume, C = {{ experience_degradation_cost }}(Q/1000)^2)
...
- Convex cost function: $C_{intern}(Q) = {{ experience_degradation_cost }} \cdot (Q/1000)^2$
...
Society bears **\${{ external_cost_per_beer }}/beer** in external costs:
- Crime & violence: \$2.50
- Public health: \$1.50

Current taxes (\${{ current_taxes_per_beer_raw }}) cover only **{{ tax_coverage_pct }}%** of these costs.

**Pigouvian tax gap: \${{ pigouvian_gap }}/beer**

### 4. Price Ceiling (\$7) Has Mixed Effects

**Pros:**
- Consumer surplus: significant gains (+\${{ ceiling7_consumer_surplus_pct }}%)
- More affordable access

**Cons:**
- Stadium profit: \${{ ceiling7_profit_change_season }}M/season
- Externality costs: increase (+\${{ ceiling7_externality_cost_pct }}%) due to higher consumption
...
Adding \${{ pigouvian_gap }}/beer tax:
- ✓ Internalizes external costs
- ✓ Raises \${{ pigouvian_revenue_annual }}M/year for affected communities
...
**Implement \${{ pigouvian_gap }}/beer additional tax on stadium alcohol**

- Consumer price: \${{ baseline_beer }} → \${{ pigouvian_consumer_price }}
- Consumption: reduces to optimal level
- Revenue: \${{ pigouvian_revenue_annual }}M/year
...
For policy: Focus on **true external costs** (crime, public health) that remain uninternalized. Current taxes cover only {{ tax_coverage_pct }}% of these costs.

**Optimal policy: \${{ pigouvian_gap }}/beer Pigouvian tax, raising \${{ pigouvian_revenue_annual }}M/year for NYC.**